...
CREATE OR REPLACE PROCEDURE update_customers(
    p_customers_to_update_or_insert INTEGER DEFAULT 1
)
LANGUAGE plpgsql
AS $$
DECLARE
    ...
    v_is_active BOOLEAN;
    v_customer_address VARCHAR(500);

    v_current_value INTEGER := 0;

    v_customer_names VARCHAR(500)[];
    v_active_statuses BOOLEAN[];
    v_customer_addresses VARCHAR(500)[];

BEGIN

    v_customer_names := ARRAY[
        'ACME INC',
        'Amazing Customer INC',
        'Amazing Partner LLC',
        'Green Forest INC',
        'Blue Sky LLC',
        'AI Customer INC',
    ...
    v_customer_addresses := ARRAY[
        'Rain Forest Street, 1234, Fantasy City, USA',
        'Blue Sky Street, 1234, Fantasy City, USA',
        'Awesome Street, 1234, Fantasy City, USA'
    ];

    WHILE v_current_value < p_customers_to_update_or_insert LOOP

        SELECT v_customer_names[
            FLOOR(RANDOM() * 20) + 1
        ]
        INTO v_customer_name;

        SELECT v_customer_addresses[
            FLOOR(RANDOM() * 3) + 1
        ]
        INTO v_customer_address;

        SELECT v_active_statuses[
            FLOOR(RANDOM() * 2) + 1
        ]
        INTO v_is_active;
...
CREATE OR REPLACE PROCEDURE update_products(
    p_products_to_update_or_insert INTEGER DEFAULT 1
)
LANGUAGE plpgsql
AS $$
DECLARE
    ...
    v_is_active BOOLEAN;
    v_barcode VARCHAR(26);
    v_unity_price DECIMAL;

    v_current_value INTEGER := 0;

    v_product_names VARCHAR(500)[];
    v_active_statuses BOOLEAN[];

BEGIN

    v_product_names := ARRAY[
        'Pencil','Pen','Screen','Laptop','Car','Truck',
        'Basketball Ball','Baseball Ball','Motorcycle',
        'Parrot','Goldfish','Rice','Beans','TV','Fridge',
        'Oven','Cooker','Desk','Chair','Wheel','Tyre',
        'Break Pads','Spot Lights','T-Shirt','Jacket',
        'Pants','Gasoline','Tickets'
    ];

    v_active_statuses := ARRAY[TRUE, FALSE];

    WHILE v_current_value < p_products_to_update_or_insert LOOP

        SELECT v_product_names[
            FLOOR(RANDOM() * 28) + 1
        ]
        INTO v_product_name;
...
-- =========================
-- GENERATE ORDERS (FIXED)
-- =========================

CREATE OR REPLACE PROCEDURE generate_orders(
    p_orders_to_generate INTEGER DEFAULT 1,
    p_itens_per_order INTEGER DEFAULT 25
)
LANGUAGE plpgsql
AS $$
DECLARE
    ...
    v_order_statuses VARCHAR(20)[];

BEGIN

    v_order_statuses := ARRAY[
        'PROCESSING',
        'COMPLETED',
        'REPROCESSING'
    ];
    ...
    WHILE v_current_order_value < p_orders_to_generate LOOP

        v_current_value := 0;

        SELECT v_customer_ids[
            FLOOR(
                RANDOM()
        ...
        IF v_order_id IS NULL THEN
            ...
            WHILE v_current_value < p_itens_per_order LOOP

                SELECT v_product_ids[
                    FLOOR(
                        RANDOM()
        ...
        v_current_order_value :=
            v_current_order_value + 1;
